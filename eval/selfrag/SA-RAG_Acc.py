...
def count_true(answer, output):
    ...
    return 1.0 * count_num_num / count_num
...
def main():
    ...
    #         answer = data['answer']
    #         answer = eval(answer)
    #         output = data_train['Output']
    #         # output = data['SA_RAG_data'][0]
    #         # isSup = data['isSup']   # 原始数据中的最原始结果
    #         result_acc += count_true(answer, output)  # 生成的回复和检索到的answer进行对比
    #         num_all += 1  # 需要检索数据的数量
    #
    # print(result_acc / num_all)
    # print(num_all)
    ...
    for data in input_datas:
        ...
        if data['Retrieval'] == "[Retrieval]":
            ...
            output = data['SA_RAG_data'][0]
            # output = data['glm-4-answer']   # 0.6671232431682592
            # isSup = data['isSup']   # 原始数据中的最原始结果
            result_acc += count_true(answer, output)  # 生成的回复和检索到的answer进行对比
            num_all += 1 # 需要检索数据的数量
