...
def main():
    ...
    # 如果启用了flash attention，对llama模型进行补丁处理
    print("______________________________364___________________________________________")
    ...
    # in the environment
    # 初始化加速器。在本例中，我们将让加速器为我们处理设备放置。
    # 如果我们使用跟踪，我们也需要在这里初始化它它会默认拾取环境中所有支持的跟踪器
    ...
    # 使用调试配置为每个进程创建一个日志。
    print("_______________________________384__________________________________________")
    logging.basicConfig(
    ...
    # llama没有默认的pad令牌!
    # 这里我们再次添加所有特殊标记，因为默认标记不在special_tokens_map中
    ...
    # 我们只在必要时调整嵌入的大小，以避免索引错误。如果您在一个小词汇表上从零开始创建一个模型，并且想要更小的嵌入尺寸，请删除此测试。
    ...
    # 注意:当前的加速器.step()调用实际调度器的.step()的' num_processes '次数。这是因为它们假设用户用整个训练集初始化调度程序。
    # 在数据并行训练的情况下，每个进程只看到训练集的一个子集(1/num_processes)。因此，每次进程都需要多次更新lr，以便最终的更新总数与这里的num_training_steps匹配。
    # Here we need to set the num_training_steps to either using the entire training set (when epochs is specified) or we need to multiply the 
    # num_training_steps by num_processes so that the total number of updates matches the num_training_steps.
    # 在这里，我们需要将num_training_steps设置为使用整个训练集(当指定epochs时)，或者我们需要将num_training_steps乘以num_processes，以便更新的总数与num_training_steps匹配。
    ...
    # 我们需要重新计算总的训练步骤，因为训练数据加载器的大小可能发生了变化。
    num_update_steps_per_epoch = math.ceil(len(train_dataloader) / args.gradient_accumulation_steps)
    ...
    if args.output_dir is not None:
        ...
        # 在进行多gpu训练时，我们需要使用accelerator.get_state_dict(model)来获取state_dict。
        # 否则，有时模型将只保存部分参数。
        # 此外，加速器需要使用包装模型来获取state_dict。
        # When doing multi-gpu training, we need to use accelerator.get_state_dict(model) to get the state_dict.
        # Otherwise, sometimes the model will be saved with only part of the parameters.
        # Also, accelerator needs to use the wrapped model to get the state_dict.
        state_dict = accelerator.get_state_dict(model)
        if args.use_lora:
            # 当使用lora时，unwrapped模型是PeftModel，它不支持is_main_process
            # 并且有自己的save_pretrained函数，只用于保存lora模块。
            # 我们必须在save_pretrained函数之外手动指定is_main_process。
