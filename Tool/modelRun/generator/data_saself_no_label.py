...
def remove_paragraph_tags(text):
    # 正则表达式匹配 <paragraph> 和 </paragraph> 之间的内容
    pattern = r'\[.*?\]'
...
def main():
    ...
    file_train_save_path = './SArag_all/SArag_train_train_no_label.jsonl'
    ...
    for data in input_datas:
        if data['Retrieval'] == "[Retrieval]":
            # data['output'] = remove_paragraph_tags(data['output'])
            data['output'] = '<paragraph>' + data['answer'] + '</paragraph>' + data['glm-4-answer']
        else:
            # data['output'] = remove_paragraph_tags(data['output'])
